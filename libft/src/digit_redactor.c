...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/12/23 14:43:13 by gdoze             #+#    #+#             */
/*   Updated: 2020/02/21 17:28:50 by ltalitha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*width_redactor(char *arg, char *dash, t_flag flags)
{
	...
	if (flags.width > len)
	{
		...
		if (flags.spcf == 'o' && *arg == '1' && ft_strlen(arg) == 1)
			ft_memset(str, '0', flags.width - len - 1);
		else
...
static char	*length_redactor(char *arg, register t_flag flags, int len)
{
	...
	if (len < flags.length)
	{
		...
		if (flags.spcf == 'd' && flags.spase && flags.filler == '0' && !dash)
			str[0] = ' ';
		arg = (flags.minus) ? ft_strjoin_free(arg, str, 3) :\
